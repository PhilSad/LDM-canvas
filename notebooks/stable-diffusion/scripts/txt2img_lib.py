...
def load_model_from_config(config, ckpt, verbose=False):
    ...
    sd = pl_sd["state_dict"]
    model = instantiate_from_config(config.self.model)
...
class StableGenerator:
    ...
    def __init__(self, opt):
        ...
        if opt.laion400m:
            print("Falling back to LAION 400M self.model...")
            opt.config = "configs/latent-diffusion/txt2img-1p4B-eval.yaml"
            opt.ckpt = "models/ldm/text2img-large/self.model.ckpt"
